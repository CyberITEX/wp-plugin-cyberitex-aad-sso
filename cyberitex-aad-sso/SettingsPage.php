...
class AADSSO_Settings_Page {
		...
		if( isset( $_GET['aadsso_migrate_from_json_status'] ) ) {
			if( 'success' === $_GET['aadsso_migrate_from_json_status'] ) {
				echo '<div id="message" class="notice notice-success"><p>'
					. __( 'Legacy settings have been migrated and the old configuration file has been deleted.', 'microsoft-entra-wpconnect' )
					. __('To finish migration, unset <code>AADSSO_SETTINGS_PATH</code> from <code>wp-config.php</code>. ', 'microsoft-entra-wpconnect')
					.'</p></div>';
			} elseif ( 'manual' === $_GET['aadsso_migrate_from_json_status'] ) {
				echo '<div id="message" class="notice notice-warning"><p>'
					. esc_html__( 'Legacy settings have been migrated successfully. ', 'microsoft-entra-wpconnect' )
					. sprintf( __('To finish migration, delete the file at the path <code>%s</code>. ', 'microsoft-entra-wpconnect'), AADSSO_SETTINGS_PATH )
					. sprintf( __('Then, unset <code>AADSSO_SETTINGS_PATH</code> from <code>wp-config.php</code>. ', 'microsoft-entra-wpconnect') )
					.'</p></div>';
			} elseif( 'invalid_json' === $_GET['aadsso_migrate_from_json_status'] ) {
				echo '<div id="message" class="notice notice-error"><p>'
					. sprintf( __('Legacy settings could not be migrated from <code>%s</code>. ', 'microsoft-entra-wpconnect'), AADSSO_SETTINGS_PATH )
					. esc_html( 'File could not be parsed as JSON. ', 'microsoft-entra-wpconnect' )
					. esc_html( 'Delete the file, or check its syntax.', 'microsoft-entra-wpconnect' )
					.'</p></div>';
			}
		}
	}
	...
	public function add_options_page() {
		$this->options_page_id = add_options_page(
			__( 'Microsoft Entra ID Settings', 'microsoft-entra-wpconnect' ), // page_title
	...
	public function register_settings() {
		...
		add_settings_section(
			...
			__( 'General', 'microsoft-entra-wpconnect' ), // title
		...
		add_settings_section(
			...
			__( 'Advanced', 'microsoft-entra-wpconnect' ), // title
		...
		add_settings_field(
			'org_display_name', // id
			__( 'Display name', 'microsoft-entra-wpconnect' ), // title
		...
		add_settings_field(
			'org_domain_hint', // id
			__( 'Domain hint', 'microsoft-entra-wpconnect' ), // title
		...
		add_settings_field(
			'client_id', // id
			__( 'Client ID', 'microsoft-entra-wpconnect' ), // title
		...
		add_settings_field(
			'client_secret', // id
			__( 'Client secret', 'microsoft-entra-wpconnect' ), // title
		...
		add_settings_field(
			'redirect_uri', // id
			__( 'Redirect URL', 'microsoft-entra-wpconnect' ), // title
		...
		add_settings_field(
			...
			__( 'Logout redirect URL', 'microsoft-entra-wpconnect' ), // title
		...
		add_settings_field(
			...
			__( 'Enable full logout', 'microsoft-entra-wpconnect' ), // title
		...
		add_settings_field(
			...
			__( 'Field to match to UPN', 'microsoft-entra-wpconnect' ), // title
		...
		add_settings_field(
			...
			__( 'Match on alias of the UPN', 'microsoft-entra-wpconnect' ), // title
		...
		add_settings_field(
			...
			__( 'Enable auto-provisioning', 'microsoft-entra-wpconnect' ), // title
		...
		add_settings_field(
			...
			__( 'Enable auto-forward to Microsoft Entra ID', 'microsoft-entra-wpconnect' ), // title
		...
		add_settings_field(
			...
			__( 'Enable Microsoft Entra ID group to WordPress role association', 'microsoft-entra-wpconnect' ), // title
		...
		add_settings_field(
			'default_wp_role', // id
			__( 'Default WordPress role if not in Microsoft Entra ID group', 'microsoft-entra-wpconnect' ), // title
		...
		add_settings_field(
			'role_map', // id
			__( 'WordPress role to Microsoft Entra ID group map', 'microsoft-entra-wpconnect' ), // title
		...
		add_settings_field(
			...
			__( 'OpenID Connect configuration endpoint', 'microsoft-entra-wpconnect' ), // title
	...
	function role_map_callback() {
		printf( '<p>%s</p>',
			__( 'Map WordPress roles to Microsoft Entra ID groups.', 'microsoft-entra-wpconnect' )
		);
		echo '<table>';
		printf(
			'<thead><tr><th>%s</th><th>%s</th></tr></thead>',
			__( 'WordPress Role', 'microsoft-entra-wpconnect' ),
			__( 'Microsoft Entra ID Group Object ID', 'microsoft-entra-wpconnect' )
		);
		echo '<tbody>';
	...
	public function org_display_name_callback()  {
		...
		printf(
			...
			__( 'Display Name will be shown on the WordPress login screen.', 'microsoft-entra-wpconnect' )
		);
	}
	...
	public function org_domain_hint_callback()  {
		...
		printf(
			...
			__( 'Provides a hint to Microsoft Entra ID about the domain or tenant they will be logging in to. If '
			     ...
			     . 'endpoint.', 'microsoft-entra-wpconnect' )
		);
	}
	...
	public function client_id_callback() {
		...
		printf(
			'<p class="description">%s</p>',
			__( 'The client ID of the Microsoft Entra ID application representing this blog.', 'microsoft-entra-wpconnect' )
		);
	}
	...
	public function client_secret_callback() {
		...
		printf(
			'<p class="description">%s</p>',
			__( 'A secret key for the Microsoft Entra ID application representing this blog.', 'microsoft-entra-wpconnect' )
		);
	}
	...
	public function redirect_uri_callback() {
		...
		printf(
			...
			. '<p class="description">%s</p>',
			wp_login_url(),
			__( 'Set default', 'microsoft-entra-wpconnect' ),
			__( 'The URL where the user is redirected to after authenticating with Microsoft Entra ID. '
			  ...
			  . 'the default value (which is this blog\'s login page).', 'microsoft-entra-wpconnect' )
		);
	}
	...
	public function logout_redirect_uri_callback() {
		...
		printf(
			...
			__( 'The URL where the user is redirected to after signing out of Microsoft Entra ID. '
			  ...
			  . ' logging out of the blog, it is only used when logging out of Microsoft Entra ID.)', 'microsoft-entra-wpconnect' )
		);
	}
	...
	public function field_to_match_to_upn_callback() {
		...
		<select name="aadsso_settings[field_to_match_to_upn]" id="field_to_match_to_upn">
			<option value="email"<?php echo $selected == 'email' ? ' selected="selected"' : ''; ?>>
				<?php echo __( 'Email Address', 'microsoft-entra-wpconnect' ); ?>
			</option>
			<option value="login"<?php echo $selected == 'login' ? ' selected="selected"' : ''; ?>>
				<?php echo __( 'Login Name', 'microsoft-entra-wpconnect' ); ?>
			</option>
		</select>
		<?php
		printf(
			...
			__( 'This specifies the WordPress user field which will be used to match to the Microsoft Entra ID user\'s '
			  . 'UserPrincipalName.', 'microsoft-entra-wpconnect' )
		);
	}
	...
	public function default_wp_role_callback() {
		...
		printf(
			...
			__('This is the default role that users will be assigned to if matching Microsoft Entra ID group to '
			 ...
			 . 'configured Microsoft Entra ID groups.', 'microsoft-entra-wpconnect')
		);
	}
