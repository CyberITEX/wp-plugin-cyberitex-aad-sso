...
/*
* Plugin Name: Microsoft Entra WPConnect
* Plugin URI: https://www.cyberitex.com
* Description: Microsoft Entra WPConnect allows you to use your organization's Microsoft Entra ID (formerly known as Azure Active Directory) user accounts to log in to WordPress. If your organization is using Office 365, your user accounts are already in Microsoft Entra ID. This plugin uses OAuth 2.0 to authenticate users and the Microsoft Graph API to retrieve group membership and other details.
* Author: CyberITEX
* Version: 1.7.1
* Author URI: https://www.cyberitex.com
* Text Domain: microsoft-entra-wpconnect
* Domain Path: /languages/
* License: This plugin is released under a proprietary license and is only for use by CyberITEX clients as per the terms and conditions outlined in the Proprietary License Agreement between CyberITEX and its clients.
*/
...
if (! defined('AADSSO')) {
	define('AADSSO', 'microsoft-entra-wpconnect');
}

if (! defined('AADSSO_PLUGIN_URL')) {
...
if (! defined('AADSSO_DEBUG')) {
	define('AADSSO_DEBUG', false);
}

if (! defined('AADSSO_DEBUG_LEVEL')) {
	define('AADSSO_DEBUG_LEVEL', 0);
}
...
class AADSSO
{
	...
	public function __construct($settings)
	{
		...
		add_shortcode('aad_microsoft_login', array($this, 'aad_microsoft_login_shortcode'));
		...
		if (isset($_GET['code'])) {
			...
			if (! isset($_SESSION['aadsso_antiforgery-id'])) {
				return new WP_Error(
					...
					__('Session does not contain antiforgery ID.', 'microsoft-entra-wpconnect')
				);
			}
			...
			if ($state_is_missing || $state_doesnt_match) {
				return new WP_Error(
					...
					sprintf(__('ANTIFORGERY_ID mismatch. Expecting %s', 'microsoft-entra-wpconnect'), $antiforgery_id)
				);
			}
			...
			if (isset($token->access_token)) {
				...
				try {
					...
					return new WP_Error(
						'invalid_id_token',
						sprintf(__('ERROR: Invalid id_token. %s', 'microsoft-entra-wpconnect'), $e->getMessage())
					);
				}
						...
						return new WP_Error(
							...
							sprintf(
								__('ERROR: Unable to check group membership with Microsoft Graph: '
									. '<b>%s</b> %s<br />%s', 'microsoft-entra-wpconnect'),
				...
				return new WP_Error(
					$token->error,
					sprintf(
						__('ERROR: Could not get an access token to Microsoft Graph. %s', 'microsoft-entra-wpconnect'),
				...
				return new WP_Error('unknown', __('ERROR: An unknown error occured.', 'microsoft-entra-wpconnect'));
			}
			...
			return new WP_Error(
				$_GET['error'],
				sprintf(
					__('ERROR: Access denied to Microsoft Graph. %s', 'microsoft-entra-wpconnect'),
	...
	function get_wp_user_from_aad_user($jwt, $group_memberships)
	{
		...
		if (is_a($user, 'WP_User')) {
			...
			if (true === $this->settings->enable_auto_provisioning) {
				...
				if (
					true === $this->settings->enable_aad_group_to_wp_role
				...
				if (is_wp_error($new_user_id)) {
					...
					return new WP_Error(
						'user_not_registered',
						sprintf(
							__('ERROR: Error creating user \'%s\'.', 'microsoft-entra-wpconnect'),
							$unique_name
						)
					);
				} else {
	...
	function update_wp_user_roles($user, $group_memberships)
	{
		...
		if (! empty($roles_to_set)) {
			...
			$error_message = sprintf(
				__('ERROR: Microsoft Entra ID user %s is not a member of any group granting a role.', 'microsoft-entra-wpconnect'),
				$aad_user_id
			);
	...
	function print_plugin_not_configured()
	{
		echo '<div id="message" class="error"><p>'
			. __('Microsoft Entra WPConnect required settings are not defined. '
				. 'Update them under Settings > Microsoft Entra ID.', 'microsoft-entra-wpconnect')
			. '</p></div>';
	}
	...
	{
		// Define the image URL for the Microsoft logo
		$image_url = plugins_url('assets/images/microsoft-5.svg', WP_PLUGIN_DIR . '/cyberitex-aad-sso/microsoft-entra-wpconnect.php');
	
		// Generate the login and logout URLs
		$login_url = $this->get_login_url();
		$logout_url = $this->get_logout_url();
	
		// Start building the button HTML
		$html = '<p class="aadsso-login-form-text">';
		$html .= '<style>/* Style for the "Sign in with Microsoft" button */
		...
		// Add the "Sign in with Microsoft" button
		$html .= '<a class="microsoft-signin-btn" href="' . esc_url($login_url) . '">';
		$html .= '<img src="' . esc_url($image_url) . '" alt="Microsoft Logo" class="microsoft-logo">';
		$html .= __('Sign in with Microsoft', 'microsoft-entra-wpconnect');
		$html .= '</a>';
	
		// Add the "Sign out" button only if the user is logged in
		if (is_user_logged_in()) {
			$html .= '<br /><a class="dim" href="' . esc_url($logout_url) . '">'
				. __('Sign out', 'microsoft-entra-wpconnect') . '</a>';
		}
	...
	public function aad_microsoft_login_shortcode() {
		// Start output buffering to capture the output of print_login_link
		ob_start();
	
		// Check if the current instance exists
		if (isset($this)) {
			$this->print_login_link();
		} else {
			echo '<p>Login link functionality is not available.</p>';
		}
